...
class ValueNode extends GroupNode
{
    ...
    public function addDoubleQuotedValue(DoubleQuotedValueNode $doubleQuotedValue): self
    {
        $this->addAttribute(NodeAttribute::fromNode($doubleQuotedValue->setParent($this)));
        return $this;
    }

    /** @return DoubleQuotedValueNode[] */
    public function getDoubleQuotedValues(): array
    {
        $result = [];
        foreach ($this->getByName('doubleQuotedValue') as $attr) {
            if ($attr instanceof NodeAttribute) {
                /** @var DoubleQuotedValueNode $node */
                $node = $attr->node;
                $result[] = $node;
            }
        }
    ...
    public function removeDoubleQuotedValue(DoubleQuotedValueNode $doubleQuotedValue): self
    {
        foreach ($this->getByName('doubleQuotedValue') as $attr) {
            if ($attr instanceof NodeAttribute && $attr->node === $doubleQuotedValue) {
                $this->removeAttribute($attr);
                break;
            }
        }
    ...
    public function addSingleQuotedValue(string $content): self
    {
        $this->addAttribute(new RawRegionAttribute(opener: '\'', content: $content, closer: '\'', name: 'singleQuotedValue', anchorName: null));
        return $this;
    }

    /** @return string[] */
    public function getSingleQuotedValues(): array
    {
        $result = [];
        foreach ($this->getByName('singleQuotedValue') as $attr) {
    ...
    public function removeSingleQuotedValueByIndex(int $index): self
    {
        $matches = $this->getByName('singleQuotedValue');
        if (isset($matches[$index])) {
            $this->removeAttribute($matches[$index]);
        }
        return $this;
