...
class Defaults
{
    ...
    /**
     * The whitespace/structural role this default materialises when it is a generic
     * trivia slot (e.g. a `-l*` group manages role `leadingWs`). `null` means the
     * role is the structural node's own name (e.g. `comma`).
     */
    public ?string $managedRole = null;
    ...
    /**
     * Fluent constructor for a trivia slot whose formatting-managed role is $role.
     */
    public static function managingRole(string $role): self
    {
        $defaults = new self();
        $defaults->managedRole = $role;
